...
namespace kfs
{
  ...
  private:
    ...
    std::string _in_fn;
    FILE* _in_stream;
    boost::unordered_map<long, int> _mer_counts;
  ...
  public:
    ...
    inline boost::unordered_map<long, int>& mer_counts(void) { return _mer_counts; }
    inline int mer_count(const long& k) { return _mer_counts[k]; }
    inline void mer_counts(const boost::unordered_map<long, int>& mc) { _mer_counts = mc; }
